CREATE TABLE IF NOT EXISTS jobs (
    ...
    next_run_at                        BIGINT         NOT NULL DEFAULT 0                COMMENT '下次预计执行时间（Unix 时间戳，秒）',
    last_run_at                        BIGINT         NOT NULL DEFAULT 0                COMMENT '上次执行时间（Unix 时间戳，秒）',
    last_success_at                    BIGINT         NOT NULL DEFAULT 0                COMMENT '上次成功执行时间（Unix 时间戳，秒）',
    created_at                         BIGINT         NOT NULL DEFAULT 0                COMMENT '创建时间（Unix 时间戳，秒）',
    updated_at                         BIGINT         NOT NULL DEFAULT 0                COMMENT '更新时间（Unix 时间戳，秒）',
    PRIMARY KEY (id),
...
CREATE TABLE IF NOT EXISTS job_runs (
    ...
    scheduled_at BIGINT       NOT NULL DEFAULT 0                         COMMENT '计划执行时间（Unix 时间戳，秒）',
    started_at   BIGINT       NOT NULL DEFAULT 0                         COMMENT '实际开始时间（Unix 时间戳，秒）',
    finished_at  BIGINT       NOT NULL DEFAULT 0                         COMMENT '结束时间（Unix 时间戳，秒）',
